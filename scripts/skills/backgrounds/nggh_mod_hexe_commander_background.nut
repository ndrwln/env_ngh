this.nggh_mod_hexe_commander_background <- ::inherit("scripts/skills/backgrounds/nggh_mod_hexe_background", {
	...
	{
		::Tactical.Entities.getFlags().set("CollectUniqueLoot_" + this.getID(), false);
	...
	{
		// call the parent function, don't mess with this
		this.nggh_mod_hexe_background.onCombatFinished();

		// check if this loot gathering has already done
		::logInfo("checking if the loot gathering is done or not");

		if (::Tactical.Entities.getFlags().get("CollectUniqueLoot_" + this.getID())) return;

		local properties = ::Tactical.State.getStrategicProperties();

		// can't loot in arena
		local isArena = !::Tactical.State.isScenarioMode() && properties != null && properties.IsArenaMode;	

		// can't loot if it's banned to do so
		local canLoot = !::Tactical.State.isScenarioMode() && properties != null && !properties.IsLootingProhibited;

		if (isArena || !canLoot) return;

		local loot = [];
		local size = ::Tactical.getMapSize();

		// iterating through all map tiles
		for( local x = 0; x < size.X; ++x )
		{
			for( local y = 0; y < size.Y; ++y )
			{
				...
				// check if there is a corpse in this tile 
				if (!tile.Properties.has("Corpse")) continue;

				// check if the corpse is edible. can be consumed = has meat
				if (!tile.Properties.get("Corpse").IsConsumable) continue;

				// add a stack strange meat to the loot table
				::logInfo("add a stack of strange meat to loot table");
				::Tactical.State.m.CombatResultLoot.add(::new("scripts/items/supplies/strange_meat_item"));

				// mark the processed corpse
				tile.Properties.get("Corpse").IsConsumable = false;
			}
		}

		// set flag to true
		::logInfo("finished the loot gathering process");
		::Tactical.Entities.getFlags().set("CollectUniqueLoot_" + this.getID(), true);
	}
